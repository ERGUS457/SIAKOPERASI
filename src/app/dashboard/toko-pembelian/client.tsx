...
import { TokoPembelian } from "@prisma/client";
import { createTokoPembelian, updateTokoPembelian, deleteTokoPembelian } from "./actions";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface TokoPembelianClientProps {
  data: TokoPembelian[];
}

export function TokoPembelianClient({ data }: TokoPembelianClientProps) {
  const [isPending, startTransition] = useTransition();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  
  const [selectedItem, setSelectedItem] = useState<TokoPembelian | null>(null);
  ...
  const handleEdit = (item: TokoPembelian) => {
  ...
  const handleDeleteClick = (item: TokoPembelian) => {
    setSelectedItem(item);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        if (selectedItem) {
          await updateTokoPembelian(selectedItem.id, formData);
        } else {
          await createTokoPembelian(formData);
        }
  ...
  const onDelete = () => {
    ...
    startTransition(async () => {
      try {
        await deleteTokoPembelian(selectedItem.id);
    ...
    <>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {selectedItem ? "Edit Toko Pembelian" : "Tambah Toko Pembelian"}
            </DialogTitle>
          </DialogHeader>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Data toko pembelian ini akan dihapus secara permanen.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

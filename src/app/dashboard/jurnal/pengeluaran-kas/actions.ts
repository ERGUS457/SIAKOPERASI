...
export async function submitJurnal(data: any) {
  ...
  if (totalDebit <= 0) throw new Error("Total pengeluaran harus lebih dari 0.");

  if (!data.akunKasId) throw new Error("Akun Sumber (Kas/Bank) tidak valid.");

  const tanggal = new Date(data.tanggal);
  const month = (tanggal.getMonth() + 1).toString().padStart(2, "0");
  const year = tanggal.getFullYear().toString();
  const prefix = getJurnalPrefix("PENGELUARAN_KAS");
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (!akunKas) {
      throw new Error("Akun Sumber (Kas/Bank) tidak ditemukan di database.");
    }
    ...
    const transaksi = await tx.transaksi.create({
      data: {
        ...
        detailJurnal: {
          create: [
            // Baris Debit: Berdasarkan input pengguna (bisa banyak, misal dari Beban, Utang)
            ...
            // Baris Kredit: Otomatis memotong Kas/Bank yang dipilih user
            {
              akunId: akunKas.id,
              posisi: "KREDIT" as PosisiJurnal,
              nominal: totalDebit,
              keterangan: data.keterangan || "Pengeluaran Kas"
            }
          ]
        }
      }
    ...
    if (anggotaId) {
      ...
      // Untuk Pengeluaran Kas, Akun Simpanan berada di sisi DEBIT (mengurangi saldo simpanan anggota)
